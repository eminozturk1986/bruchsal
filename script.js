...
Which museum is located along the Museumsufer and focuses on fine arts?,Museum für Moderne Kunst,Senckenberg Museum,Historisches Museum,Städel Museum,Städel Museum,50.103,8.6742`;
        ...
        try {
            ...
            const query = `
                ...
                (
                  ...
                  way["highway"~"^(primary|secondary|tertiary|trunk)$"]["name"]
    ...
    drawMajorStreets8Bit(ctx, width, height, streets, bounds) {
        const streetsByType = {
            primary: { color: '#8B4513', width: 6 }, // Brown for major roads
            secondary: { color: '#A0522D', width: 4 }, // Sandy brown for secondary
            tertiary: { color: '#D2691E', width: 3 }, // Chocolate for smaller roads
            trunk: { color: '#654321', width: 7 } // Dark brown for highways
        };
        
        // Draw streets by importance
        Object.keys(streetsByType).forEach(streetType => {
            const style = streetsByType[streetType];
            ...
            streets.filter(street => street.type === streetType).forEach(street => {
                if (street.geometry && street.geometry.length > 1) {
                    ...
                    if (streetType === 'primary' || streetType === 'trunk') {
                        if (midPoint && street.name && street.name !== 'Street') {
                            ctx.fillStyle = '#FFFFFF';
                            ...
                            // Draw text outline
                            ctx.strokeText(street.name, midPoint.x, midPoint.y - 8);
                            ctx.fillText(street.name, midPoint.x, midPoint.y - 8);
                        }
                    }
                }
            });
