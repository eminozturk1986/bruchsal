...
Which museum is located along the Museumsufer and focuses on fine arts?,Museum für Moderne Kunst,Senckenberg Museum,Historisches Museum,Städel Museum,Städel Museum,50.103,8.6742`;
    ...
    async loadRealStreets(centerLat, centerLng, radiusKm = 1) {
        try {
            // Use Overpass API to get real street data from OpenStreetMap
            const overpassUrl = 'https://overpass-api.de/api/interpreter';
            const query = `
                [out:json][timeout:25];
                (
                  way["highway"~"^(primary|secondary|tertiary|residential|trunk|unclassified)$"]
    ...
    async draw8BitStreets(ctx, width, height) {
        ...
        // Try to load real streets if we have user location
        if (this.userLocation) {
            streetData = await this.loadRealStreets(this.userLocation.lat, this.userLocation.lng, 0.5);
        }
        
        if (streetData && streetData.length > 0) {
            // Draw real streets in 8-bit style
            this.drawRealStreets8Bit(ctx, width, height, streetData);
        } else {
    ...
    drawRealStreets8Bit(ctx, width, height, streetData) {
        const centerX = width / 2;
        const centerY = height / 2;
        
        // Calculate bounds for coordinate conversion
        const bounds = this.calculateBounds(streetData);
        ...
        streetData.forEach(way => {
            if (way.geometry && way.geometry.length > 1) {
                ...
                way.geometry.forEach(node => {
                    const pixelCoords = this.geoToPixel(
                        node.lat, node.lon, 
                        bounds, width, height
                    );
        ...
        // Add some 8-bit buildings around streets
        this.addRandomBuildings8Bit(ctx, width, height, streetData, bounds);
        ...
        ctx.font = '8px monospace';
        ctx.textAlign = 'center';
        ctx.fillText('YOUR AREA', centerX, centerY + 25);
    }
    ...
    drawGenericStreets8Bit(ctx, width, height) {
        ...
        ctx.fillStyle = '#daa520';
        ctx.fillRect(width/2 - 10, width/2 - 10, 20, 20);
        
        ctx.fillStyle = '#ffffff';
        ctx.font = '8px monospace';
        ctx.textAlign = 'center';
        ctx.fillText('GENERIC AREA', width/2, height - 10);
    }
}
